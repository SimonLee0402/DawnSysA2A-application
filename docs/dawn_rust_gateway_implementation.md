...
When a settlement request is actually submitted, DawnCore now validates the amount against the advertised AP2 pricing metadata before it creates the local AP2 transaction.
...
- Agent Card quote support is currently metadata-driven; there is not yet a live remote quote negotiation round-trip or signed quote object exchanged between agents.
